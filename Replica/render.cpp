...
int main(int argc, char* argv[]) {
  ...
  FILE *fp=fopen("processed_poses.txt", "r"); 
  // Render some frames
  const size_t numFrames = 200000;
  const size_t startFrame = 0;
  for (size_t i = 0; i < startFrame; i++) {
    for (int i=0; i<16; i++){
      static float tmp;
  ...
  for (size_t i = startFrame; i < numFrames; i++) {
    ...
    for (int i=0; i<16; i++){
      ...
      s_cam.GetModelViewMatrix().m[i]=tmp;
    }
